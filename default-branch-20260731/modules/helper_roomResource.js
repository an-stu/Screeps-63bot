...
function printRichHtml(html) {
    if (typeof console.logUnsafe == "function") console.logUnsafe(html);
    else console.log(String(html).replace(/<[^>]*>/g, " ").replace(/\s+/g, " "));
}
...
global.roomResSvg=(res, allCnt,len)=>{
    ...
    let ratio = allCnt ? left / allCnt * 100 : 0;
    return `<svg width="${len}px" height="8px"><rect width="${len}" height="8" fill="black"/>${svgs}</svg> ${_.padLeft(ratio.toFixed(1),4)}% ${_.padLeft((allCnt/1000000).toFixed(1),4)}M  `
}
